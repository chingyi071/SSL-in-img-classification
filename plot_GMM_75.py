...
final_dotx   = []
final_doty   = []
base_filename = 'report/SSL-clf-DNN-slt-GMM%d75-match-label600.txt'
for param, linestyle in zip([50,60,70,80], [':','--','-.','-']):
  ...
  lc1 = LineCollection(segments1, linewidths=2.0, color=color1,linestyle=linestyle, label='prob th = '+str(param))

  ax.add_collection(lc1)
  final_points.append((float(points1[-1][0][0]), float(points1[-1][0][1])+0.2, "th(%d): %.1lf%%(%d samples)" % (param,float(points1[-1][0][1]),int(points1[-1][0][0]))))
...
plt.title('Probability threshold (perc th 75%)',fontsize='x-large',fontweight='bold')
plt.legend()
