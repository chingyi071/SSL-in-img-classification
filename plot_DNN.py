...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 23:10:07 2019
...
with open('/Users/shadow_iris/Desktop/SSL4/report/SSL-clf-DNN-slt-mhigh-DNN80-match-label600.txt') as csv_file1:
  ...
  for item in csv_reader1:
      if csv_reader1.line_num == 1 or csv_reader1.line_num >= 22:
        continue
      if csv_reader1.line_num == 21:
        Test_accuracy1.append(float(item[5]))
      else:#Train_labeled_loss.append(item[0])
...
#######################################6010#########################
with open('/Users/shadow_iris/Desktop/SSL4/report/SSL-clf-DNN-slt-naive-DNN80-match-label600.txt') as csv_file7:
#csv_file=f.read()
...
#  Test_accuracy7.append(0.85)
  ...
  for item in csv_reader7:
      if csv_reader7.line_num == 1 or csv_reader7.line_num >= 22:
        continue
...
for i in range(len(New_label_acc7)):
    if float(New_label_acc7[i]) <=1 and float(New_label_acc7[i])>0.95:
        color7.append('#000080') #navy
    if float(New_label_acc7[i])<=0.95 and float(New_label_acc7[i])>0.9:
        color7.append('#006400') #darkgreen
    if float(New_label_acc7[i])<=0.9 and float(New_label_acc7[i])>0.85:
        color7.append('#FF7F50') #coral
#    if float(New_label_acc1[i])<=0.7 and float(New_label_acc1[i])>0.6:
#        color1.append('#A52A2A')
    else:
        color7.append('#DC143C') #crimson
...
##############################################4015###########################
with open('/Users/shadow_iris/Desktop/SSL4/report/SSL-clf-DNN-slt-mhigh-DNN90-match-label600.txt') as csv_file2:
#csv_file=f.read()
...
#  Test_accuracy2.append(0.85)
  ...
  for item in csv_reader2:
      if csv_reader2.line_num == 1 or csv_reader2.line_num >= 22:
        continue
...
for i in range(len(New_label_acc2)):
    if float(New_label_acc2[i]) <=1 and float(New_label_acc2[i])>0.95:
        color2.append('#000080') #navy
    if float(New_label_acc2[i])<=0.95 and float(New_label_acc2[i])>0.9:
        color2.append('#006400') #darkgreen
    if float(New_label_acc2[i])<=0.9 and float(New_label_acc2[i])>0.85:
...
#########################4020#################################################
with open('/Users/shadow_iris/Desktop/SSL4/report/SSL-clf-DNN-slt-naive-DNN90-match-label600.txt') as csv_file3:
#csv_file=f.read()
...
#  Test_accuracy3.append(0.85)
  ...
  for item in csv_reader3:
      if csv_reader3.line_num == 1 or csv_reader3.line_num >= 22:
        continue
...
plt.ylabel('Accuracy')
plt.title('(Prob, Percentile) for DNN  re-label',fontsize='xx-large',fontweight='bold')
